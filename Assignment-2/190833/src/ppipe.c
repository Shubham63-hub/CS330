...
struct ppipe_info* alloc_ppipe_info() {
    ...
    ppipe->ppipe_global.totalbuffempty = MAX_PPIPE_SIZE;
    
    //initialising all ppipe_info_per_process 
    int i=0;
    while(i<MAX_PPIPE_PROC){
        ...
        ppipe->ppipe_per_proc[i].buffempty = MAX_PPIPE_SIZE;
        i++;
    }

    //first process connected to ppipe
...
int do_ppipe_fork (struct exec_context *child, struct file *filep) {
    ...
    int flag1 = 0;
    int parentindex ;
    while(j<MAX_PPIPE_PROC){
        if(filep->ppipe->ppipe_per_proc[j].pid == parent->pid && filep->ppipe->ppipe_per_proc[j].used == 1){
            parentindex = j;
            flag1 = 1;
            break;
        }
    ...
    // printk("fork called\n");
    while(i<MAX_PPIPE_PROC){  
        // printk("child: %d, given: %d, used: %d, read: %d\n", child->pid, filep->ppipe->ppipe_per_proc[i].pid, filep->ppipe->ppipe_per_proc[i].used, filep->ppipe->ppipe_per_proc[i].read_state);
    ...
    while(i<MAX_PPIPE_PROC){
        if(flag1){
            if(filep->mode == O_READ){
                ...
                filep->ppipe->ppipe_per_proc[index].read_pointer = filep->ppipe->ppipe_per_proc[parentindex].read_pointer;
                filep->ppipe->ppipe_per_proc[index].buffempty =  filep->ppipe->ppipe_per_proc[parentindex].buffempty;
            }
            ...
            if(filep->mode == O_READ){
                ...
                filep->ppipe->ppipe_per_proc[i].read_pointer = filep->ppipe->ppipe_per_proc[parentindex].read_pointer;
                filep->ppipe->ppipe_per_proc[i].buffempty = filep->ppipe->ppipe_per_proc[parentindex].buffempty;
            }
            if(filep->mode == O_WRITE){
                // printk("write called\n");
...
long ppipe_close (struct file *filep) {
    ...
    if(flag){
        if(filep->mode == O_READ){
            if(filep->ppipe->ppipe_per_proc[index].read_state == -1){
                return -EOTHERS;
            }
            filep->ppipe->ppipe_per_proc[index].read_state = -1;
        }
        if(filep->mode == O_WRITE){
            if(filep->ppipe->ppipe_per_proc[index].write_state == -1){
                return -EOTHERS;
            }
            filep->ppipe->ppipe_per_proc[index].write_state = -1;
        }
        ...
        int i = 0;
        while( i<MAX_PPIPE_PROC){
            if(filep->ppipe->ppipe_per_proc[i].read_state == -1 && filep->ppipe->ppipe_per_proc[i].write_state == -1){
                filep->ppipe->ppipe_per_proc[i].used = -1;
            }
...
int do_flush_ppipe (struct file *filep) {
    ...
    int minpointer = MAX_PPIPE_SIZE , maxpointer = 0;
    int i = 0;
    int flag = 1;
    while(i<MAX_PPIPE_PROC){
        // printk("pid= %d, readopen: %d, used: %d\n",filep->ppipe->ppipe_per_proc[i].pid, filep->ppipe->ppipe_per_proc[i].read_state, filep->ppipe->ppipe_per_proc[i].used );
        if(filep->ppipe->ppipe_per_proc[i].used == 1 && filep->ppipe->ppipe_per_proc[i].read_state == 1){
            ...
            flag = 0;
            // printk("max: %d, min: %d total: %d\n", maxpointer, minpointer,filep->ppipe->ppipe_global.totalbuffempty);
        }
        i++;
    }
    if(flag){
    ...
    // printk("min: %d, max: %d, leftpointer :%d\n", minpointer, maxpointer, filep->ppipe->ppipe_global.left_pointer);
    ...
    if(filep->ppipe->ppipe_global.left_pointer<maxpointer && filep->ppipe->ppipe_global.left_pointer>minpointer){
        ...
        filep->ppipe->ppipe_global.totalbuffempty += reclaimed_bytes;
    }
    else if(filep->ppipe->ppipe_global.left_pointer>maxpointer && filep->ppipe->ppipe_global.left_pointer>minpointer){
        reclaimed_bytes = MAX_PPIPE_SIZE - filep->ppipe->ppipe_global.left_pointer + minpointer;
        ...
        if(filep->ppipe->ppipe_global.totalbuffempty == 0){
            int minbuffempty = MAX_PPIPE_SIZE;
            // printk("me called\n");
            for(int i = 0; i<MAX_PPIPE_PROC; i++){
                if(minbuffempty >= filep->ppipe->ppipe_per_proc[i].buffempty && filep->ppipe->ppipe_per_proc[i].used == 1 && filep->ppipe->ppipe_per_proc[i].read_state == 1){
                    minbuffempty = filep->ppipe->ppipe_per_proc[i].buffempty;
                }
            }
            reclaimed_bytes = minbuffempty;
            filep->ppipe->ppipe_global.left_pointer = minpointer;
            filep->ppipe->ppipe_global.totalbuffempty += reclaimed_bytes;
        }
        else{
        ...
        reclaimed_bytes = 0;
        filep->ppipe->ppipe_global.left_pointer = maxpointer;
        filep->ppipe->ppipe_global.totalbuffempty += reclaimed_bytes;
    } 
    else if(filep->ppipe->ppipe_global.left_pointer<maxpointer && filep->ppipe->ppipe_global.left_pointer==minpointer){
        reclaimed_bytes = 0;
...
int ppipe_read (struct file *filep, char *buff, u32 count) {
    ...
    while( i<MAX_PPIPE_PROC){
        ...
        // printk("used: %d : pid: %d : given: %d: read: %d: write: %d \n",filep->ppipe->ppipe_per_proc[i].used, filep->ppipe->ppipe_per_proc[i].pid, current->pid, filep->ppipe->ppipe_per_proc[i].read_state, filep->ppipe->ppipe_per_proc[i].write_state);
    ...
    while( i<count ){
        ...
        filep->ppipe->ppipe_per_proc[index].buffempty += 1;
        if(filep->ppipe->ppipe_per_proc[index].buffempty > MAX_PPIPE_SIZE){
            filep->ppipe->ppipe_per_proc[index].buffempty = MAX_PPIPE_SIZE;
            break;
        }

        if(filep->ppipe->ppipe_per_proc[index].read_pointer >= MAX_PPIPE_SIZE){
    ...
    // for(int i = 0; i<MAX_PPIPE_PROC; i++){
    //     if(filep->ppipe->ppipe_per_proc[i].read_state == 1 && filep->ppipe->ppipe_per_proc[i].used == 1){
    //         printk("readstate: %d, buffempty:%d\n", filep->ppipe->ppipe_per_proc[i].read_pointer, filep->ppipe->ppipe_per_proc[i].buffempty);
    //     }
    // }
...
int ppipe_write (struct file *filep, char *buff, u32 count) {
    ...
    int bytes_written = 0;
    i = 0;
    // printk("total: %d\n",filep->ppipe->ppipe_global.totalbuffempty );
    while( i<count ){
         
        filep->ppipe->ppipe_global.totalbuffempty -= 1;
        if(filep->ppipe->ppipe_global.totalbuffempty < 0){
            filep->ppipe->ppipe_global.totalbuffempty = 0;
            for(int i =0 ; i<MAX_PPIPE_PROC;i++){
                if(filep->ppipe->ppipe_per_proc[i].read_state == 1 && filep->ppipe->ppipe_per_proc[i].used == 1) 
                    filep->ppipe->ppipe_per_proc[i].buffempty -= bytes_written;
            }
            break;
        }
    ...
    // for(int i = 0;i <MAX_PPIPE_PROC; i++){
    //     if(filep->ppipe->ppipe_per_proc[i].read_state == 1){
    //         printk("readstate: %d, buffempty:%d\n", filep->ppipe->ppipe_per_proc[i].read_pointer, filep->ppipe->ppipe_per_proc[i].buffempty);
    //     }
    // }
...
int create_persistent_pipe (struct exec_context *current, int *fd) {
    ...
    //making file object and ppipe info object
    ...
    if(ppipeinfo == NULL) return -ENOMEM;
