...
int main (u64 arg1, u64 arg2, u64 arg3, u64 arg4, u64 arg5) {
	...
	if (!pid) {                 // Child 1 code.
        ...
        // Read 3096 characters from the ppipe.
        char read_buffer[3097];
        ret_code = read (fd[0], read_buffer, 3096);
        read_buffer[3096] = 'b';
        if (ret_code < 0) {
            ...
            // Expected result should be 3096 and the last character in the read
            // buffer should be 'v' and then next should be 'b'.
            printf ("%d\n", ret_code);
            printf ("%c\n", read_buffer[3095]);
            printf ("%c\n", read_buffer[3096]);
        ...
        if (!pid_2) {           // Child 2 code.
            ...
            // Call flush which will return 0.
            ret_code = flush_ppipe (fd);
            if (ret_code < 0) {
            
                printf ("Child 2: Flush on Persistent Pipe is failed!!!\n");
                return -1;
    ...
    // Read 1000 characters from the ppipe.
    char read_buffer_2[1001];
    read_buffer_2[1000] = 'r';
    ...
    if (ret_code < 0) {
        ...
        // Expected result should be 1000 and the last character in the read
        // buffer should be 's' and next should be 'r';
        printf ("%d\n", ret_code);
        printf ("%c\n", read_buffer_2[999]);
        printf ("%c\n", read_buffer_2[1000]);
    ...
    // Call flush which will return 1000.
    ret_code = flush_ppipe (fd);
    if (ret_code < 0) {
