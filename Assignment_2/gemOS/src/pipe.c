...
struct pipe_info_global {
    ...
    // TODO:: Add members as per your need...
    int read_pointer;   //points to the point till which read is done(including the point)
...
int is_valid_mem_range (unsigned long buff, u32 count, int access_bit) {
    ...
    if(access_bit == 1){
        ...
        while(i<MAX_MM_SEGS-1){
            if(current->mms[i].start<=buff && current->mms[i].next_free >= buff+count && (current->mms[i].access_flags)%2 == 1){
                return 1;
            }
            i++;
        }
        if(current->mms[MAX_MM_SEGS-1].start<=buff && current->mms[MAX_MM_SEGS-1].end>=buff+count && (current->mms[MAX_MM_SEGS-1].access_flags)%2 == 1){
            return 1;
        }
        while(current->vm_area != NULL){
            if(current->vm_area->vm_start<=buff && current->vm_area->vm_end>=buff+count && (current->vm_area->access_flags)%2 ==1){
                return 1;
            }
        ...
        int i = 0;
        while(i<MAX_MM_SEGS-1){
            if(current->mms[i].start<=buff && current->mms[i].next_free >= buff+count && (current->mms[i].access_flags)%4 - (current->mms[i].access_flags)%2 == 2){
                return 1;
            }
            i++;
        }
        if(current->mms[MAX_MM_SEGS-1].start<=buff && current->mms[MAX_MM_SEGS-1].end>=buff+count && (current->mms[MAX_MM_SEGS-1].access_flags)%4 - (current->mms[MAX_MM_SEGS-1].access_flags)%2 == 2){
            return 1;
        }
        while(current->vm_area != NULL){
            if(current->vm_area->vm_start<=buff && current->vm_area->vm_end>=buff+count && ((current->vm_area->access_flags)%4 - (current->vm_area->access_flags)%2 )== 2){
                return 1;
            }
            current->vm_area = current->vm_area->vm_next;
        }
...
int pipe_read (struct file *filep, char *buff, u32 count) {
    ...
    i = 0;
    while( i<count ){
        if(filep->pipe->pipe_global.read_pointer == filep->pipe->pipe_global.write_pointer){
            break;
        }
...
int pipe_write (struct file *filep, char *buff, u32 count) {
    ...
    i = 0;
    while( i<count ){
        if(filep->pipe->pipe_global.read_pointer == (filep->pipe->pipe_global.write_pointer + 1)%4097){
            break;
        }   
